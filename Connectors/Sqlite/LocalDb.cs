...
public class SqliteLocal : ILocalDb
{
    ...
    public string GetSqlScript(ISchema schema)
    {
        ...
        foreach (var table in schema.Tables)
        {
            ...
            // create table
            sb.Append($"CREATE TABLE [{table.TableName}] (").AppendLine();
            ...
            foreach (var column in table.Columns)
            {
                ...
                sb.Append(',').AppendLine().Append('[').Append(column.ColumnName).Append(']');
                ...
                if ((column.Requirement & XRequirementOptions.Required) != 0)
                {
                    ...
                    {
                        case "TEXT":
                            sb.Append(' ').Append($"CHECK ({column.ColumnName} <> '')");
                            break;
                        case "INTEGER":
                        case "REAL":
                            sb.Append(' ').Append($"CHECK ({column.ColumnName} <> 0)");
                            break;
                    }
                }
            }
            ...
            foreach (var index in table.Indexes)
            {
                ...
                var indexName = $"{index.IndexName}_{table.TableName}";
                var indexOn = new StringBuilder();
                foreach (var column in index.Columns)
                {
                    if (indexOn.Length > 0) indexOn.Append(", ");
                    indexOn.Append('[').Append(column.ColumnName).Append(']');
                }
                sb.Append($"CREATE UNIQUE INDEX [{indexName}]").AppendLine();
                sb.Append($"ON [{table.TableName}] ({indexOn})").AppendLine();
            }
        }
